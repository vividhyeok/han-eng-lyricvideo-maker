...
def _split_title_artist(title: str, fallback_artist: str = "") -> Dict[str, str]:
    if not title:
        return {"title": "", "artist": fallback_artist}
    for sep in [" - ", " ? ", " ? "]:
        if sep in title:
            artist, t = title.split(sep, 1)
            return {"title": t.strip(), "artist": artist.strip()}
    return {"title": title, "artist": fallback_artist}
...
class YTMusicHandler:
    ...
    def search_songs(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 2. Fetch from general search (unfiltered)
            results_broad = self.yt.search(query, limit=limit)

            # 2.5 Fetch from "videos" filter to catch items only listed as videos
            results_videos = self.yt.search(query, filter="videos", limit=limit)
            ...
            # 4. Merge results (Strict first, then Broad, then Videos)
            combined_results = []
            seen_ids = set()
            ...
            print(
                f"DEBUG: [V2_MERGED] Found {len(results_strict)} strict, "
                f"{len(filtered_broad)} broad, {len(results_videos)} videos. "
                f"Total merged: {len(combined_results)}"
            )
            ...
            max_score = max((calculate_relevance(query, x) for x in formatted), default=0)
            if not formatted or max_score < 40:
                # Fallback to YouTube search when YTMusic is empty or irrelevant (common for remixes)
                yt_results = youtube_search(query)
                seen_urls = {x.get('youtube_url') for x in formatted if x.get('youtube_url')}
                for res in yt_results:
                    url = res.get('link', '')
                    if not url or url in seen_urls:
                        continue
                    split = _split_title_artist(res.get('title', ''), fallback_artist=query)
                    formatted.append({
                        'title': split['title'],
                        'artist': split['artist'],
                        'album': 'YouTube',
    ...
    def get_album_tracks(self, browse_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, track in enumerate(tracks):
                ...
                # Fallback: If videoId is missing, try searching for the track in YTMusic
                if not video_id:
                    try:
                ...
                if not youtube_url:
                    # Secondary fallback to YouTube search
                    try:
                        search_query = f"{artist_names} {track.get('title')}".strip()
                        yt_results = youtube_search(search_query)
                        if yt_results:
                            youtube_url = yt_results[0].get('link')
