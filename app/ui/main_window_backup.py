...
class WorkerThread(QThread):
    ...
    upload_requested = pyqtSignal(str, str, str)  # video_path, title, artist
...
class MainWindow(QMainWindow):
    ...
    def on_youtube_upload_toggled(self, state):
        ...
        print(f"[DEBUG] YouTube 자동 업로드: {'활성화' if self.youtube_upload_enabled else '비활성화'}")

    def on_upload_requested(self, video_path, title, artist):
        """YouTube 업로드 요청 처리"""
        self.progress_window.close()
        
        # YouTube 업로드 다이얼로그 표시
    ...
    def on_model_changed(self, index):
        ...
        if model_id:
            ...
            print(f"[DEBUG] Translation model changed to: {model_id}")
    ...
    def create_input_section(self):
        ...
        model_layout.addWidget(QLabel("번역 AI 모델:"))
        ...
        if not available_models:
            self.model_combo.addItem("No AI models available (check API keys)")
        ...
        self.youtube_upload_checkbox = QCheckBox("YouTube에 자동 업로드")
        self.youtube_upload_checkbox.setStyleSheet("font-weight: bold; color: #d32f2f;")
        self.youtube_upload_checkbox.stateChanged.connect(self.on_youtube_upload_toggled)
        input_layout.addWidget(self.youtube_upload_checkbox)
